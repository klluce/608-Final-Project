...
# Will a specific vehicle of the top 5 body types receive any web leads (interested buyers filling out a form for more information on the vehicles page)?

# ## Inferential Question

# Is there an association between Buyer Engagements (The amount of clicks on a vehicles web page) and Web Leads (specific clicks where an interested buyer fills out a form requesting for information on a specific vehicle) for all WTS vehicles?
...
# What is the population mean estimate for price for all WTS service trucks? 
...
# 
# Repeat steps 1–3 (sampling with replacement) until you have 
#  observations, which form a bootstrap sample.
# 
# Calculate the bootstrap point estimate (e.g., mean, median, proportion, slope, etc.) of the 
#  observations in your bootstrap sample.
# 
...
#limiting to the top 5 body types (no other category)
upfits_subset3 = upfits_subset.copy()

service_only = upfits_subset3.query('body_type == "Service Truck"')
...
one_sample = service_only.sample(n=40)
one_sample
...
one_sample_dist = alt.Chart(one_sample).mark_bar().encode(
    x=alt.X("final_price")
        .bin(maxbins=30)
        .title("Price of Service Truck (dollars)"),
...
mean = boot1["final_price"].mean()

mean_line = alt.Chart(pd.DataFrame({"mean": [mean]})).mark_rule(
    color="black",
    strokeWidth=2,
    strokeDash=[5, 5]    # dotted pattern
).encode(
    x="mean:Q"
)
...
mean_label = alt.Chart(pd.DataFrame({
    ...
    align="left",
    baseline="bottom",
    dx=5,        # horizontal offset from line
    dy=-5,       # vertical offset
    color="red",
    fontSize=12
).encode(
    x="mean:Q",
    y=alt.value(0),   # place at top of chart
    text="label:N"
)

chart_with_mean = boot1_dist + mean_line + mean_label
...
(
    ...
    .mean()
    .reset_index()
    .rename(columns={"final_price": "mean_price"})
)
...
boot_est_dist = alt.Chart(boot20000_means,
    title="Bootstrap Distribution"
).mark_bar().encode(
    x=alt.X("mean_price")
        .bin(maxbins=20)
        .title("Sample mean price of Service Truck (dollars)"),
    y=alt.Y("count()").title("Count"),
)
...
mean_est = boot20000_means["mean_price"].mean()
ci_bounds = boot20000_means["mean_price"].quantile([0.025, 0.975])
lo, hi = ci_bounds.iloc[0], ci_bounds.iloc[1]
ci_bounds
...
mean_line = alt.Chart(pd.DataFrame({"x": [mean_est]})).mark_rule(
    color="red",
    strokeWidth=2,
    strokeDash=[5, 5]
).encode(
...
ci_lines = alt.Chart(ci_df).mark_rule(
    color="blue",
    strokeWidth=2,
    strokeDash=[4, 4]
).encode(
...
ci_labels = alt.Chart(ci_df).mark_text(
    align="left",
    baseline="bottom",
    dx=4,
    dy=-20,
...
final_chart = boot_est_dist + mean_line + mean_label + ci_lines + ci_labels
final_chart
...
samples = pd.concat([
    service_only.sample(40).assign(replicate=n)
    for n in range(20_000)
])
samples
...
sampling_distribution = alt.Chart(sample_estimates,
    title="Sampling Distribution").mark_bar().encode(
    x=alt.X("mean_price")
        .bin(maxbins=30)
        .title("Sample mean of price of Service Truck (dollars)"),
    y=alt.Y("count()").title("Count")
)

mean3 = sample_estimates["mean_price"].mean()

mean_line3 = alt.Chart(pd.DataFrame({"x": [mean3]})).mark_rule(
    color="red",
    strokeWidth=2,
    strokeDash=[5, 5]
).encode(
...
mean_label3 = alt.Chart(pd.DataFrame({
    "x": [mean3],
    "label": [f"Mean = ${mean3:,.0f}"]
})).mark_text(
    align="left",
    baseline="bottom",
    dx=5,
...
final_chart3 = sampling_distribution + mean_line3 + mean_label3
final_chart3
...
# Comparing the two, we see that the shape and spread of the true sampling distribution and the bootstrap distribution are similar; the bootstrap distribution lets us get a sense of the point estimate’s variability. The second important point is that the means of these two distributions are slightly different. The sampling distribution is centered at 67,321 dollars, the population mean value. However, the bootstrap distribution is centered at the original sample’s mean price, 68,660 dollars. Because we are resampling from the original sample repeatedly, we see that the bootstrap distribution is centered at the original sample’s mean value (unlike the sampling distribution of the sample mean, which is centered at the population parameter value).

# Here the sample mean price of 40 Service Trucks was 68,660 dollars, and our 95 percent Confidence interval is between 65,808 dollars and $71,734. Notice that our interval does indeed contain the true population mean value, 67,321 dollars.
