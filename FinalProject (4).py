...
# Visually inspecting the scatter plot above, it appears that the more buyer engagements there are, the more vehicles seem to receive more than one web lead.
...
upfits_train, upfits_test = train_test_split(
    upfits_subset, train_size=0.75, stratify=upfits_subset["received_lead"]
)
upfits_train.info()
...
upfits_preprocessor = make_column_transformer(
    (StandardScaler(), ["buyer_engagements", "searches_body_type"]),
)
...
X = upfits_train[["buyer_engagements", "searches_body_type"]]
...
# For example, the model predicts that a vehicle with 1000 clicks on its page and 100,000 searches by the vehicles body type will receive at least 1 web lead, while a vehicle with 250 clicks and the same amount of searches will not receive any web leads.

# In[63]:


upfits_test["predicted"] = knn_pipeline.predict(upfits_test[["buyer_engagements", "searches_body_type"]])
...
knn_pipeline.score(
    upfits_test[["buyer_engagements", "searches_body_type"]],
...
# The confusion matrix shows that 699 observations were correctly predicted as receiving at least 1 web lead, and 5,596 were correctly predicted as receiving none. 1,320 were predicted as receiving no web leads, when they actually received one, a false negative, and 861 were predicted to have received at least 1 when they received none, a false positive.

# Since 76% of our dataset received no web leads, the majority classifier would predict all tested vehicles as receiving no web leads. Thus, the estimated accuracy of a majority estimator would be about 76%. Since our model has an accuracy of 74%, it performs slightly worse. 
